...
Change the URL if yours is different. Keep it short — the limits are what stop
the board filling with noise.
...
- **Card title: 8 words max. Log entry: one sentence.** Never paste code,
  output, errors, or file lists. The board is for glancing at.
- Skip the board entirely for questions, small edits, dead ends, and anything
  I did not ask you to build. One task, one card — not one per step.

If it is not worth reading in a week, do not write it. Writing less is the
safer mistake.
